...
from twitchdrives.exceptions import (
    VehicleAsleep,
    VehicleError,
    VehicleInvalidShare,
    VehicleTimeout,
...
async def get_tesla():
    return await AsyncTesla.create(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "Tesla/1.0.0"},
    )
...
class Vehicle(dict):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while True:
                async with session.ws_connect(
                    STREAMING_BASE_URL + "streaming/", heartbeat=3
                ) as ws:
                    await ws.send_json(
                        {
    ...
    async def api(self, name, **kwargs):
        ...
        api_response = await self.tesla.api(
            name, {"vehicle_id": self["id_s"]}, **kwargs
        )
...
class AsyncTesla(AsyncOAuth2Client):
    ...
    async def _refresh_token(
        ...
        **kwargs
        ...
        resp = await self.post(url, json=body, headers=headers, auth=auth, **kwargs)
    ...
    async def load_token(self) -> dict:
        ...
        if token:
            ...
            await self.refresh_token(
                SSO_BASE_URL + "oauth2/v3/token", os.environ["TESLA_TOKEN"]
            )
    ...
    async def api(self, name, path_vars=None, **kwargs):
        ...
        request = await self.request(
            method=endpoint["TYPE"], url=uri, **{arg_name: kwargs}
        )
